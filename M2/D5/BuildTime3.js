...
function findJob(location, position) {
  let foundJobs = [];
  let num = 0;
  for (let i = 0; i < jobs.length; i++) {
    const element = jobs[i];
    if (element.location.toLowerCase().includes(location.toLowerCase()) &&
      (element.title.toLowerCase().includes(position.toLowerCase()) ||
        element.function.toLowerCase().includes(position.toLowerCase()) ||
        element.department.toLowerCase().includes(position.toLowerCase()))) {
      foundJobs.push({
